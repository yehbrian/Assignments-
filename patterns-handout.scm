...
(define (repeat-rows nrepeats pattern)
    (make-pattern 
     ...
     (* nrepeats (pattern-numrows pattern))
     (lambda (rows cols) ((pattern-fn pattern) (modulo rows (pattern-numrows pattern)) cols))
     )
  )

(define (get-rows-length first second)
  (+ (pattern-numrows first) (pattern-numrows second))
  )

(define (get-cols-length first second)
  (+ (pattern-numcols first) (pattern-numcols second))
  )

(define (min-row first second)
  (min (pattern-numrows first) (pattern-numrows second))
  )

(define (min-col first second)
  (min (pattern-numcols first) (pattern-numcols second))
  )



;;; append cols returns the pattern made by appending second to the right of first
;;; the number of rows in the resulting pattern is the smaller of the number of rows in first and patten2
(define (append-cols first second)
  (make-pattern
   (min-row first second) 
   (get-cols-length first second)
   (lambda (rows cols)
     (if (> (pattern-numcols first) cols)
         ((pattern-fn first) rows cols)
         ((pattern-fn second) rows (- cols (pattern-numcols first)))
         )
     )
   )
)

;;; append-rows returns the pattern made by appending second below first
;;; the number of columns in the resulting pattern is the smaller of the number of columns in first and patten2
(define (append-rows first second)
  (make-pattern
   (get-rows-length first second) 
   (min-col first second)
   (lambda (rows cols)
     (if (> (pattern-numrows first) rows)
         ((pattern-fn first) rows cols)
         ((pattern-fn second)(- rows (pattern-numrows first))cols)
         )
     )
   )
  )
...
(make-pattern
 ...
 (lambda (rows cols)
   ((pattern-fn pattern)
    rows
    (- (pattern-numcols pattern) (+ cols 1) )
    )
   )
 )
)
...
(define (flip-rows pattern)
  (make-pattern
   ...
   (lambda (rows cols)
     ((pattern-fn pattern) 
      (- (pattern-numrows pattern) (+ rows 1)) cols
      )
     )
   )
  )